...
func buildCertFromASN1(derData []byte, rawCert *asn1Certificate) *x509.Certificate {
	...
	// 解析签名算法（TBS 中的 signatureAlgorithm）
	if len(rawCert.TBS.SigAlgo.FullBytes) > 0 {
		var sigAlgoOID asn1.ObjectIdentifier
		if _, err := asn1.Unmarshal(rawCert.TBS.SigAlgo.FullBytes, &sigAlgoOID); err == nil {
			cert.SignatureAlgorithm = oidToSignatureAlgorithm(sigAlgoOID)
		} else {
			// AlgorithmIdentifier 是 SEQUENCE { OID, params }，需要先解外层
			var seq asn1.RawValue
			if _, err := asn1.Unmarshal(rawCert.TBS.SigAlgo.FullBytes, &seq); err == nil && seq.IsCompound {
				if _, err := asn1.Unmarshal(seq.Bytes, &sigAlgoOID); err == nil {
					cert.SignatureAlgorithm = oidToSignatureAlgorithm(sigAlgoOID)
				}
			}
		}
...
// parseSubjectPublicKeyInfo 从 SubjectPublicKeyInfo DER 中提取公钥算法和公钥对象。
func parseSubjectPublicKeyInfo(cert *x509.Certificate, spkiDER []byte) {
	// 尝试使用 x509.ParsePKIXPublicKey 标准解析
	if pub, err := x509.ParsePKIXPublicKey(spkiDER); err == nil {
		cert.PublicKey = pub
		switch pub.(type) {
		case *rsa.PublicKey:
		...
		// 保存 RawSubjectPublicKeyInfo 供后续使用
		cert.RawSubjectPublicKeyInfo = spkiDER
		return
	}

	// 标准解析失败，从 ASN.1 结构中手动提取算法 OID
	...
	var s spki
	if _, err := asn1.Unmarshal(spkiDER, &s); err != nil {
		return
	}

	oidStr := s.Algorithm.Algorithm.String()
	switch oidStr {
...
func parseCertManual(derData []byte) (*x509.Certificate, error) {
	...
	// 7. subjectPublicKeyInfo (SEQUENCE) - 解析公钥
	tbsBytes, err = asn1.Unmarshal(tbsBytes, &elem)
	if err != nil {
		return cert, nil
	}
	parseSubjectPublicKeyInfo(cert, elem.FullBytes)
